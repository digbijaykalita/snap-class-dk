...
from src.components.dialogue_attendance_result import attendance_result_dialog
from src.components.dialogue_voice_attendance import voice_attendance_dialog
...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button("Logout", type="secondary", key='logoutkbtn', shortcut="control+backspace"):
...
def teacher_tab_take_attendance():
    ...
    with c2:
        ...
        if st.button('Run face analysis', type='secondary', width='stretch', icon=':material/delete:', disabled= not has_photos):
            with st.spinner('Scanning classroom photos...'):
                all_detected_id = {}
                for idx, img in enumerate(st.session_state.attendance_images):
                    imp_np = np.array(img.convert('RGB'))
                    ...
                    if not enrolled_students:
                        st.warning('No student enrolled in the course')
                    else:
    ...
    with c3:
        if st.button('Voice Input', type='primary', width='stretch', icon=':material/mic:'):
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username = st.text_input("Enter your Username", placeholder="anand123")
...
def register_teacher(teacher_name, teacher_username, teacher_pass, teacher_pass_confirm):
    ...
    try:
        ...
        return False, f"Unexpecter error! {e}"
...
def teacher_screen_register():
    ...
    teacher_name = st.text_input("Enter your Name", placeholder="Anand Kumar")
    teacher_username = st.text_input("Enter your Username", placeholder="anand123")
